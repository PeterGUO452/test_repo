...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuequality != cur->valuequality)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuequality Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagquality = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagquality)
    {
        devStatusTmp.valuequality = gizY2X(quality_RATIO,  quality_ADDITION, dataPoints->valuequality); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuequality,quality_LEN);
        byteFieldOffset += quality_LEN;
    }
